...
namespace OpenXmlStreaming.Tests;
...
public class OpenXmlPackageWriterTests
{
    ...
    [Test]
    public void CreateWord_RoundTrips()
    {
        using var ms = new MemoryStream();

        using (var writer = StreamingDocument.CreateWord(ms, WordprocessingDocumentType.Document, leaveOpen: true))
        {
            writer.WritePart(
                new("/word/document.xml", UriKind.Relative),
                "application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml",
    ...
    [Test]
    public void CreateSpreadsheet_RoundTrips()
    {
        using var ms = new MemoryStream();

        using (var writer = StreamingDocument.CreateSpreadsheet(ms, SpreadsheetDocumentType.Workbook, leaveOpen: true))
        {
            writer.WritePart(
    ...
    [Test]
    public void CreatePresentation_RoundTrips()
    {
        using var ms = new MemoryStream();

        using (var writer = StreamingDocument.CreatePresentation(ms, PresentationDocumentType.Presentation, leaveOpen: true))
        {
            writer.WritePart(
